...
const ReportView: React.FC<ReportViewProps> = ({ record, originalRecord, userRole, onBack, updateRecord }) => {
    ...
    if (userRole === UserRole.DOCTOR) {
        ...
        return (
            <div className="max-w-4xl mx-auto space-y-6 animate-fade-in-up">
                <button onClick={onBack} className="flex items-center gap-2 text-sm text-primary font-semibold hover:text-blue-700 transition-colors bg-white/50 px-4 py-2 rounded-lg shadow-sm w-fit border border-slate-200/50 backdrop-blur-sm">
    ...
    return (
        <div className="max-w-3xl mx-auto space-y-6 animate-fade-in-up mt-6">
            <button onClick={onBack} className="flex items-center gap-2 text-sm text-primary font-semibold hover:text-blue-700 transition-colors bg-white/50 px-4 py-2 rounded-lg shadow-sm border border-slate-200/50 backdrop-blur-sm w-fit">
                <ArrowLeftIcon className="h-4 w-4" />
                Back to Dashboard
            </button>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <ReportField label="Severity">
                            <span className={`px-3 py-1 rounded-md text-sm font-bold uppercase tracking-wider inline-flex items-center gap-1.5 shadow-sm ${record.aiReport?.severity === 'Low' ? 'bg-emerald-100 text-emerald-800 border-emerald-200' :
                                ...
                                <span className={`inline-block w-2 h-2 rounded-full ${record.aiReport?.severity === 'Low' ? 'bg-emerald-600' : record.aiReport?.severity === 'Medium' ? 'bg-amber-600' : 'bg-red-600'}`}></span>
