...
CONSTANT_DOCTOR_EMAIL = "doc.kri@gmail.com"

# Create tables and seed specific doctors
with app.app_context():
    try:
        ...
        seeded_doctors = [
            {
                "email": "doc.gopi.123@gmail.com",
                "password": "123456",
                "name": "Dr. Gopinath V",
                "doctor_code": "MEDZORA-DOC-GOPI",
                "district": "Dharmapuri",
                "state": "Tamil Nadu"
            },
            {
                "email": "doc.kri@gmail.com",
                "password": "123456",
                "name": "Dr. Krithika V",
                "doctor_code": "MEDZORA-DOC-KRI",
                "district": "Ooty",
                "state": "Tamil Nadu"
            },
            {
                "email": "doc.alex@gmail.com",
                "password": "12356",
                "name": "Dr. Alex M",
                "doctor_code": "MEDZORA-DOC-ALEX",
                "district": "Namakkal",
                "state": "Tamil Nadu"
            }
        ]
        
        for doc_data in seeded_doctors:
            doc_user = User.query.filter_by(email=doc_data["email"]).first()
            if not doc_user:
                hashed_pw = generate_password_hash(doc_data["password"])
                doc_user = User(email=doc_data["email"], password_hash=hashed_pw, role="DOCTOR", doctor_code=doc_data["doctor_code"]) # type: ignore
                db.session.add(doc_user)
                db.session.commit()
                
                doc_profile = Profile( # type: ignore
                    user_id=doc_user.id,
                    name=doc_data["name"],
                ...
                db.session.commit()
                print(f"Doctor '{doc_data['name']}' created successfully.")
            else:
                # Update properties if they already exist to ensure location is correct
                if not doc_user.doctor_code:
                    doc_user.doctor_code = doc_data["doctor_code"]
                    db.session.commit()
                profile = Profile.query.filter_by(user_id=doc_user.id).first()
                if profile:
                    profile.name = doc_data["name"]
                    profile.district = doc_data["district"]
                    profile.state = doc_data["state"]
                    db.session.commit()
                    
        print("Database tables and seeded doctors verified.")
    except Exception as e:
...
def signup():
    ...
    if role == 'DOCTOR':
        # Generate a unique connection code for the new doctor in real time
        import uuid
        assigned_doctor_code = f"DOC-{str(uuid.uuid4().hex)[0:8].upper()}" # type: ignore
    else:
        # Patients pass their chosen doctor's connection code to link up
        assigned_doctor_code = data.get('doctorCode')
    
    new_user = User( # type: ignore
        email=data['email'], 
        password_hash=hashed_pw, 
        role=role,
        doctor_code=assigned_doctor_code
    )
    db.session.add(new_user)
    db.session.commit()
    
    new_profile = Profile( # type: ignore
        user_id=new_user.id,
        name=data['name'],
        age=data.get('age'),
...
def get_doctors_list():
    try:
        ...
        result = []
        for doc in doctors:
            profile = Profile.query.filter_by(user_id=doc.id).first()
            name = profile.name if profile else "Unknown Doctor"
            if doc.doctor_code:
                # Format location safely stringing District & State together
                district_str = profile.district if profile and profile.district else "Central"
                state_str = profile.state if profile and profile.state else "Medical District"
                location_str = f"{district_str}, {state_str}".strip(', ')
                ...
                result.append({
                    ...
                    "rating": 4.9, # Mocking a high default rating
                    "imageUrl": f"https://api.dicebear.com/7.x/notionists/svg?seed={doc.id}&backgroundColor=eef5fc"
                })
...
def nearby_medicals(current_user):
    ...
    if not SERP_API_KEY or SERP_API_KEY == "YOUR_SERP_API_KEY":
        return jsonify({'error': 'Nearby pharmacy service is not configured. Missing SERP_API_KEY.'}), 500
    ...
    try:
        ...
        if response.status_code != 200:
            return jsonify({'error': 'Nearby pharmacy provider is unavailable right now.'}), 502

        results = response.json()
        print(f"Results keys: {results.keys()}")
        if "error" in results:
            print(f"SearchAPI.io Error: {results['error']}")
            return jsonify({'error': f"Nearby pharmacy provider error: {results['error']}"}), 502
