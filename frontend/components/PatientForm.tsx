        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input type="text" value={patientData.name} onChange={e => setPatientData({...patientData, name: e.target.value})} className="mt-1 block w-full rounded-md border-slate-300 dark:border-slate-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-base bg-slate-50 dark:bg-slate-700" />
            </div>
            <div>
              <label className="block text-base font-medium text-slate-700 dark:text-slate-300">Age</label>
              <input type="number" value={patientData.age} onChange={e => setPatientData({...patientData, age: e.target.value})} className="mt-1 block w-full rounded-md border-slate-300 dark:border-slate-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-base bg-slate-50 dark:bg-slate-700" />
            </div>
            <div>
              <label className="block text-base font-medium text-slate-700 dark:text-slate-300">Gender</label>
              <select value={patientData.gender} onChange={e => setPatientData({...patientData, gender: e.target.value as 'Male' | 'Female' | 'Other'})} className="mt-1 block w-full rounded-md border-slate-300 dark:border-slate-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-base bg-slate-50 dark:bg-slate-700">
                  <option>Male</option>
             ...
             <div>
              ...
              <input type="email" value={patientData.email} onChange={e => setPatientData({...patientData, email: e.target.value})} className="mt-1 block w-full rounded-md border-slate-300 dark:border-slate-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-base bg-slate-50 dark:bg-slate-700" />
            </div>
          </div>
          <div>
            ...
            <div className="mt-2 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {COMMON_SYMPTOMS.map(symptom => (
                <label key={symptom} className="flex items-center space-x-2 text-base text-slate-600 dark:text-slate-400">
                  <input type="checkbox" checked={patientData.symptoms.includes(symptom)} onChange={() => handleSymptomChange(symptom)} className="rounded text-blue-600 focus:ring-blue-500 border-slate-400 dark:border-slate-500 bg-slate-100 dark:bg-slate-600" />
                  <span>{symptom}</span>
                </label>
              ))}
            </div>
          ...
          <div>
            ...
            <input type="text" value={patientData.otherSymptoms} onChange={e => setPatientData({...patientData, otherSymptoms: e.target.value})} placeholder="e.g., Skin rash, joint pain" className="mt-1 block w-full rounded-md border-slate-300 dark:border-slate-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-base bg-slate-50 dark:bg-slate-700" />
          </div>
          <div>
            ...
            <textarea rows={3} value={patientData.medicalHistory.startsWith('data:') ? '' : patientData.medicalHistory} onChange={e => setPatientData({...patientData, medicalHistory: e.target.value})} disabled={!!patientData.medicalHistoryFileName} placeholder="e.g., Diabetes, Hypertension" className="mt-1 block w-full rounded-md border-slate-300 dark:border-slate-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-base bg-slate-50 dark:bg-slate-700 disabled:bg-slate-200"></textarea>
          </div>
          <div>
            <label className="block text-base font-medium text-slate-700 dark:text-slate-300">Current Medications</label>
            <textarea rows={3} value={patientData.currentMedications} onChange={e => setPatientData({...patientData, currentMedications: e.target.value})} placeholder="e.g., Metformin 500mg, Aspirin 81mg" className="mt-1 block w-full rounded-md border-slate-300 dark:border-slate-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-base bg-slate-50 dark:bg-slate-700"></textarea>
          </div>
