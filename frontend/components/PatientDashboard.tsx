                ...
                <div className="p-4 bg-slate-50">
                    ...
                    {isRequestingFollowUp ? (
                        ...
                        <div className="flex justify-between items-center mt-4">
                            ...
                            <div className="flex gap-2">
                                <Button variant="default" onClick={handleSuggestPharmacies} className="bg-primary hover:bg-blue-700 text-white shadow-premium border-none" disabled={isLoadingPharmacies}>
                                    {isLoadingPharmacies ? (
                                    ...
                                    {pharmacies.map((pharmacy, idx) => (
                                        <a key={idx}
                                            ...
                                            <div className="flex justify-between items-start mb-1">
                                                ...
                                                {pharmacy.rating && (
                                                    <span className="text-xs bg-amber-50 text-amber-700 px-2 py-0.5 rounded font-medium ml-2 whitespace-nowrap">Rating {pharmacy.rating}</span>
                                                )}
                                            </div>
                                            {pharmacy.address && (
                            ...
                            <div className="mt-8">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                    <a href="https://www.apollo247.com/specialtys/pharmacy" target="_blank" rel="noopener noreferrer"
                                        className="flex flex-col items-center justify-center p-3 rounded-xl border border-slate-200 bg-white hover:border-blue-300 hover:shadow-md transition-all group">
                                        ...
                                        <span className="font-semibold text-sm text-slate-700 group-hover:text-blue-700">Apollo 24|7</span>
                                    </a>
                                    ...
                                    <a href="https://pharmeasy.in/" target="_blank" rel="noopener noreferrer"
                                        ...
                                        <span className="font-semibold text-sm text-slate-700 group-hover:text-blue-700">PharmEasy</span>
                                    </a>
...
const PatientDashboard: React.FC<PatientDashboardProps> = ({ patientProfile, records, notifications, addOrUpdateRecord, addChatMessage, updatePatientProfile, dismissNotification, selectedRecord, setSelectedRecord, user, onLogout, authToken, apiStatus, dbStatus, userName }) => {
    ...
    const verifiedRecords = records
        .filter(r => String(r.patientData.id) === String(patientProfile.id) && r.status === ReportStatus.VERIFIED)
    ...
    return (
        <div className="flex h-screen bg-slate-50 overflow-hidden relative font-sans">
            ...
            <div className="flex-1 flex flex-col md:ml-80 ml-24 h-screen overflow-hidden animate-fade-in-up transition-all duration-300">
                <Header
                    onLogout={onLogout}
                    userId={user.id}
                    userName={userName}
                ...
                <div className="flex-1 overflow-y-auto px-4 sm:px-6 lg:px-10 py-8">
                    ...
                    {selectedRecord ? (
                        <div className="space-y-6">
                            <button onClick={handleBackToDashboard} className="text-primary hover:text-blue-700 font-medium flex items-center space-x-2 transition-colors mb-4 bg-white/50 px-4 py-2 rounded-lg shadow-sm border border-slate-200/50 backdrop-blur-sm w-fit hover:bg-white/80">
                        ...
                        <div className="space-y-8">
                            ...
                            {activeTab === 'home' && (
                                <div className="space-y-6">
                                    <Card className="border-none bg-white">
                                        <CardHeader className="bg-gradient-to-r from-blue-50/50 to-transparent border-none pb-4">
                                            <CardTitle className="text-3xl text-primary flex items-center gap-3">
                                                <span className="p-2.5 bg-blue-50 rounded-2xl"><PlusIcon className="w-6 h-6 text-primary" /></span>
                                                New Consultation
                                            </CardTitle>
                                                ...
                                                <div>
                                                    ...
                                                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                                                        {COMMON_SYMPTOMS.map(symptom => (
                                                            <label key={symptom} className={`flex items-center gap-2.5 px-3 py-2.5 rounded-lg border text-sm cursor-pointer transition-colors ${patientData.symptoms.includes(symptom) ? 'bg-blue-50 border-primary text-primary font-medium' : 'border-slate-200 text-slate-600 bg-slate-50 hover:bg-white hover:border-slate-300'}`}>
                                                    ...
                                                    <div className="space-y-4">
                                                        ...
                                                        {patientData.medicalHistoryFileName && (
                                                            <div className="flex items-center justify-between px-3 py-2 rounded-lg bg-slate-100 border border-slate-200">
                                                                <p className="text-sm text-slate-700 truncate">Attached file: <span className="font-medium">{patientData.medicalHistoryFileName}</span></p>
                                    ...
                                    <Card className="border-none bg-white">
                                        <CardHeader className="pb-4 border-b border-slate-100/50">
                                            <div className="flex justify-between items-center">
                                                ...
                                                <Button variant="ghost" size="sm" onClick={() => setIsEditingProfile(!isEditingProfile)} className="h-9 px-3 text-primary bg-blue-50/50 hover:bg-blue-100 rounded-lg">
                            ...
                            {activeTab === 'emergencies' && (() => {
                                ...
                                const colorMap: Record<string, { bg: string; border: string; text: string; step: string; tag: string }> = {
                                    ...
                                    teal: { bg: 'bg-blue-50', border: 'border-blue-200', text: 'text-blue-800', step: 'bg-blue-500', tag: 'bg-blue-100 text-blue-700' },
                            ...
                            {activeTab === 'vision-triage' && (
                                <div className="space-y-6 animate-fade-in-up">
                                    <div className="flex items-center gap-3 border-b border-slate-200/60 pb-4 mb-6">
                                        ...
                                        <div>
                                            <h2 className="text-3xl font-bold text-blue-800 outfit tracking-tight">Vision Analyzer</h2>
