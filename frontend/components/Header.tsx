...
const Header: React.FC<HeaderProps> = ({ onLogout, userId, userName, userRole, reportStatus, notifications = [], dismissNotification, apiStatus, dbStatus }) => {
  ...
  return (
    <header className="w-full bg-white/95 backdrop-blur border-b border-slate-200 sticky top-0 z-20">
      <div className="px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          <div className="flex items-center space-x-4">
            <h1 className="text-xl font-semibold text-slate-800 tracking-tight flex items-center gap-3 md:hidden">
              <span className="w-8 h-8 rounded-lg bg-blue-600 text-white flex items-center justify-center text-sm shadow-sm">M</span>
              Medzora
            </h1>
            <div className="flex flex-col">
              <h2 className="hidden md:block text-xl font-semibold text-slate-800 tracking-tight">
                Dashboard
              </h2>
              <div className="flex items-center gap-3 mt-1">
                <div className="flex items-center gap-1.5" title={dbStatus ? "Database connected" : "Database disconnected"}>
