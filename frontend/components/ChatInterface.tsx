  ...
  return (
    <div className="flex flex-col h-[500px] bg-white shadow-premium rounded-3xl overflow-hidden relative">
        ...
        {messages.length === 0 ? (
          ...
          messages.map(msg => (
            <div key={msg.id} className={`flex items-end gap-3 ${msg.senderId === currentUserId ? 'justify-end' : 'justify-start'} animate-fade-in-up`}>
              <div className={`max-w-[75%] md:max-w-md lg:max-w-lg px-5 py-3 shadow-sm ${msg.senderId === currentUserId ? 'bg-primary text-white rounded-2xl rounded-br-sm' : 'bg-blue-50 text-slate-800 rounded-2xl rounded-bl-sm border-none'}`}>
                <p className="text-sm leading-relaxed">{msg.text}</p>
                <div className={`mt-2 flex items-center gap-1.5 ${msg.senderId === currentUserId ? 'justify-end text-blue-100' : 'justify-start text-slate-400'}`}>
                  <span className="text-[10px] font-medium tracking-wide">
                    {new Date(msg.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                  </span>
                  {msg.senderId === currentUserId && <span className="text-[10px]">Sent</span>}
                </div>
              </div>
            </div>
          ))
      ...
      <form onSubmit={handleSend} className="p-4 bg-white/80 border-t border-slate-100/50 flex items-center gap-3 backdrop-blur-md">
        ...
        />
        <Button type="submit" className="rounded-xl px-6 py-3 shadow-md bg-gradient-to-r from-primary to-blue-600 hover:from-primary hover:to-blue-700 hover:shadow-lg transition-all border-none font-bold tracking-wide">
          Send
        </Button>
      </form>
    </div>
